...
class Ros2Subscriber(PosedRgbdDataset):
    ...
    def __init__(
        ...
        src_coord_system="flu",
        pose_to_camera_translation=None,
        min_depth_m=None,
        max_depth_m=None,
        ...
        Args:
          ...
          depth_topic: Topic containing depth images of type sensor_msgs/msg/Image
            with 32FC1 or 16UC1 encoding.
          depth_scale: Scale factor to convert depth to meters. RealSense D435
            outputs mm, so use 1000.0. ZED outputs meters, so use 1.0.
          use_compressed_rgb: If True, subscribe to a CompressedImage topic
            (JPEG/PNG) instead of a raw Image topic for RGB. The rgb_topic
            should point to the compressed variant, e.g.
            ".../image_raw/compressed".
          use_compressed_depth: If True, subscribe to a CompressedImage topic
            (compressedDepth PNG) instead of a raw Image topic for depth. The
            depth_topic should point to the compressedDepth variant, e.g.
          ...
          pose_to_camera_translation: Optional xyz translation in meters from
            the pose sensor frame to the camera frame, expressed in
            src_coord_system axes. Use this when odometry comes from a sensor
            mounted with a fixed offset from the camera.
          min_depth_m: Optional minimum valid depth in meters. Depth values below
            this threshold are ignored.
          max_depth_m: Optional maximum valid depth in meters. Depth values above
            this threshold are ignored.
          pose_msg_type: Type of pose message. Either "PoseStamped" for
            geometry_msgs/PoseStamped or "Odometry" for nav_msgs/Odometry.
          frame_skip: See base.
          interp_mode: See base.
        """
        super().__init__(
        ...
        if point_cloud_topic is not None and disparity_topic is not None:
            raise ValueError(
                "You cannot set both the point cloud topic and "
        ...
        self.f = 0
        self._sync_cb_count = 0
        self._sync_cb_t0 = None
        self._sync_cb_log_period = 10  # Log every N synced messages
        ...
        if pose_to_camera_translation is None:
            pose_to_camera_translation = (0.0, 0.0, 0.0)
        if len(pose_to_camera_translation) != 3:
            raise ValueError("pose_to_camera_translation must have 3 values (x, y, z)")
        self._pose_to_camera_transform = torch.eye(4, dtype=torch.float)
        self._pose_to_camera_transform[:3, 3] = torch.tensor(
            pose_to_camera_translation, dtype=torch.float
        )
        ...
        if max_depth_m is not None and max_depth_m <= 0:
            raise ValueError("max_depth_m must be > 0")
        if (
            min_depth_m is not None
            and max_depth_m is not None
            and min_depth_m >= max_depth_m
        ):
        ...
        self._use_compressed_rgb = use_compressed_rgb
        self._use_compressed_depth = use_compressed_depth
        pose_type = PoseStamped if pose_msg_type == "PoseStamped" else Odometry
        rgb_msg_type = CompressedImage if use_compressed_rgb else Image
        depth_msg_type = CompressedImage if use_compressed_depth else Image
        msg_str_to_type = OrderedDict(
            rgb=rgb_msg_type,
            pose=pose_type,
            disp=DisparityImage,
            depth=depth_msg_type,
            pc=PointCloud,
            conf=Image,
        )
        self._topics = [
        ...
        self._time_sync = message_filters.ApproximateTimeSynchronizer(
            list(self._subs.values()), queue_size=10, slop=0.05, allow_headerless=False
        )
        ...
        logger.info("  src_coord_system: %s", src_coord_system)
        logger.info(
            "  pose_to_camera_translation (FLU): %s", list(pose_to_camera_translation)
        )
        logger.info("  pose_to_camera_transform:\n%s", self._pose_to_camera_transform)
        logger.info("  src2rdf_transform:\n%s", self.src2rdf_transform)
    ...
    def _buffer_frame_msgs(self, *msgs):
        now = time.time()
        if self._sync_cb_t0 is None:
            self._sync_cb_t0 = now
        self._sync_cb_count += 1
        elapsed = now - self._sync_cb_t0
        if self._sync_cb_count % self._sync_cb_log_period == 0 and elapsed > 0:
            rate = self._sync_cb_count / elapsed
            logger.info(
                "Sync callback rate: %.2f Hz (%d msgs in %.1f s) | Queue: %d/%d",
                rate,
                self._sync_cb_count,
                elapsed,
                self._frame_msgs_queue.qsize(),
                self._frame_msgs_queue.maxsize,
            )
        ...
        while True:
            ...
            try:
                ...
                if not self._shutdown_event.is_set():
                    logger.warning("Queue empty for 2s, no synced messages received")
                    continue
            ...
            try:
                if self._use_compressed_rgb:
                    ...
                    raw_img = compressed_image_to_numpy(msgs["rgb"])
                    rgb_np = raw_img[..., ::-1]  # BGR -> RGB
                else:
                    # Raw Image: handle different encodings
                    raw_img = image_to_numpy(msgs["rgb"])
                    encoding = msgs["rgb"].encoding
                    # Take first 3 channels (handles rgba/bgra)
                    if raw_img.ndim == 3 and raw_img.shape[2] > 3:
                        raw_img = raw_img[..., :3]
                    # Convert to RGB based on encoding
                    if encoding.startswith("bgr"):
                        rgb_np = (
                            raw_img[..., (2, 1, 0)] if raw_img.ndim == 3 else raw_img
                        )
                    else:
                        rgb_np = raw_img
            except ValueError as e:
                logger.warning(f"Skipping frame {frame_count}: {e}")
                frame_count += 1
                continue

            # Make contiguous copy and normalize to [0, 1]
            rgb_np = np.ascontiguousarray(rgb_np).astype(np.float32) / 255.0
            rgb_img = torch.from_numpy(rgb_np).permute(2, 0, 1)

            # Parse Pose (handle both PoseStamped and Odometry message types)
            if self._pose_msg_type == "Odometry":
                pose_msg = msgs[
                    "pose"
                ].pose.pose  # Odometry has pose nested in pose.pose
            else:
                pose_msg = msgs["pose"].pose  # PoseStamped has pose directly
            raw_pose_4x4 = torch.tensor(pose_to_numpy(pose_msg), dtype=torch.float)
            src_pose_4x4 = raw_pose_4x4 @ self._pose_to_camera_transform
            rdf_pose_4x4 = g3d.transform_pose_4x4(src_pose_4x4, self.src2rdf_transform)

            # Debug: log first 3 frames to trace the full pose pipeline
            if frame_count < 3:
                raw_t = raw_pose_4x4[:3, 3].tolist()
                cam_t = src_pose_4x4[:3, 3].tolist()
                rdf_t = rdf_pose_4x4[:3, 3].tolist()
                logger.info(
                    f"[POSE DEBUG frame {frame_count}] "
                    f"raw_odom(FLU)=[{raw_t[0]:.3f}, {raw_t[1]:.3f}, {raw_t[2]:.3f}]  "
                    f"cam_pose(FLU)=[{cam_t[0]:.3f}, {cam_t[1]:.3f}, {cam_t[2]:.3f}]  "
                    f"cam_pose(RDF)=[{rdf_t[0]:.3f}, {rdf_t[1]:.3f}, {rdf_t[2]:.3f}]"
                )

            if "depth" in msgs.keys():
                try:
                    if self._use_compressed_depth:
                        depth_img = compressed_depth_to_numpy(msgs["depth"])
                    else:
                ...
                # Convert from sensor units (e.g., mm) to meters
                ...
                pc_xyz_homo = g3d.transform_points_homo(
                    pc_xyz_homo, self.src2rdf_transform
                )
                ...
                if "kp_u" in ch_n2i and "kp_v" in ch_n2i:
                    u = torch.tensor(
                        msgs["pc"].channels[ch_n2i["kp_u"]].values, dtype=torch.int32
                    )
                    v = torch.tensor(
                        msgs["pc"].channels[ch_n2i["kp_v"]].values, dtype=torch.int32
                    )
                else:
                ...
                raise ValueError(
                    "Expected at least a depth or a disparity or point cloud topic"
                )

            if self._min_depth_m is not None:
                depth_img[depth_img < self._min_depth_m] = torch.nan
            if self._max_depth_m is not None:
                depth_img[depth_img > self._max_depth_m] = torch.nan
            ...
            if (
                self.depth_h != depth_img.shape[-2]
                or self.depth_w != depth_img.shape[-1]
            ):
                ...
                if conf_img is not None:
                    conf_img = torch.nn.functional.interpolate(
                        conf_img.unsqueeze(0),
                        size=(self.depth_h, self.depth_w),
                        mode="nearest-exact",
                    ).squeeze(0)
            ...
            if torch.sum(~depth_img.isnan()) == 0:
                ...
                continue
            frame_data = dict(
                rgb_img=rgb_img, depth_img=depth_img, pose_4x4=rdf_pose_4x4
            )
...
class RosnpyDataset(PosedRgbdDataset):
    ...
    def __init__(
        ...
        if os.path.isdir(path):
            self._data_files = [
                os.path.join(path, x) for x in os.listdir(path) if x.endswith(".npz")
            ]
    ...
    def __iter__(self):
        ...
        while True:
            ...
            # TODO: Verify
            flu_pose_Rt_3x4 = np.concatenate(
                (flu_pose_R, flu_pose_t.reshape(3, 1)), axis=1
            )
            flu_pose_Rt_3x4 = torch.tensor(flu_pose_Rt_3x4, dtype=torch.float)
            flu_pose_4x4 = g3d.mat_3x4_to_4x4(flu_pose_Rt_3x4)

            rdf_pose_4x4 = g3d.transform_pose_4x4(flu_pose_4x4, self._flu2rdf_transform)
